  ...
  **/
  ...
  function processServerErrors(saveContext, error) {
    ...
    error.entityErrors = serverErrors.map(function (serr) {
      ...
      console.log("EXTEND"); //FIXME
      console.log(serr); //FIXME
      var entityError = __extend({
        entity: entity,
        isServerError: true
      }, serr, ["errorName", "errorMessage", "propertyName", "severity"]);
      return entityError;
    });
  }
...
//  }
   ...
   * MINE MINE MINE
   */
  proto.mmySaveChanges = function (params, saveOptions, callback, errorCallback) {
    assertParam(params, "params").isOptional().check();
    assertParam(saveOptions, "saveOptions").isInstanceOf(SaveOptions).isOptional().check();
    assertParam(callback, "callback").isFunction().isOptional().check();
    assertParam(errorCallback, "errorCallback").isFunction().isOptional().check();
...
console.log(dataService);
    ...
    var saveBundle = { entities: params, saveOptions: saveOptions };


    try { // Guard against exception thrown in dataservice adapter before it goes async
      return dataService.adapterInstance.mySaveChanges(saveContext, saveBundle)
          .then(saveSuccess).then(null, saveFail);
    } catch (err) {
      if (errorCallback) errorCallback(err);
      return Q.reject(err);
...
;breeze.AbstractDataServiceAdapter = (function () {
  ...
  proto.mySaveChanges = function (saveContext, saveBundle) {
    console.log('myAjax');
    var adapter = saveContext.adapter = this;
    var deferred = Q.defer();
